...
export default function AddAnimal() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      if (!isAdmin)
        return addToast("You're not an admin", { appearance: "error" });

      Array.from(e.target.files).forEach(async (file) => {
        if (!file)
          return addToast("File does not exist.", { appearance: "error" });

        if (file.size > 1024 * 1024)
          // 1mb
          return addToast(
            `Size of ${file.name} too large! Upload less than 1 mb`,
            { appearance: "error" }
          );
        ...
        if (file.type !== "image/jpeg" && file.type !== "image/png")
          // 1mb
          return addToast(
            `File format for ${file.name} is incorrect. Upload .jpeg or .png, got ${file.type}`,
            { appearance: "error" }
          );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (images.length === 0)
      return addToast("At least 1 photo of the animal is required.", {
        appearance: "warning",
      });
    const toPost = { ...animal, dob: animal.dob, images };
    axios
      .post("/api/animals", toPost, {
  ...
  return (
    <div className="center">
      <form className="form" onSubmit={handleSubmit}>
        ...
        <DatePicker
          selected={animal.dob}
          onChange={(date) => setAnimal({ ...animal, dob: date })}
          style={{ width: "100%" }}
