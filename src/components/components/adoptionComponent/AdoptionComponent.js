...
export default function AdoptionComponent({
  ...
  const handleStatusChange = (id, status) => {
    axios
      ...
      .then((res) => {
        addToast("Status Changed. An email has been sent as well...", {
          appearance: "success",
        });
        refresh();
      })
        ...
        <div className="right-side">
          ...
          <h2>
            {status === 0
              ...
              : `By ${by}・${moment(updatedAt).fromNow()}`}
          </h2>
          <h4>
            {isPublic
              ...
              : {
                  ...
                  1: `Let us know how’s your life with ${
                    name.split(" ")[0]
                  } is going...`,
                  2: `Reason: ${reason}`,
                }[status]}
          ...
          <h5>
            Request {{ 0: "Raised", 1: "Approved", 2: "Rejcted" }[status]}:{" "}
            {moment(createdAt).format("MMMM Do, YYYY・hh:mm a ")}
          </h5>
          <span
            style={{ textDecoration: "underline", cursor: "pointer" }}
            onClick={() => setIsUserMessageModalOpen(true)}
          >
            User Message
          </span>
          {isUserMessageModalOpen && (
            ...
            >
              <p style={{ textAlign: "left" }}>
                <strong>{requesterName}</strong> says:
                <br />
              </p>
